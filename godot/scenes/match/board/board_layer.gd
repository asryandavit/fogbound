...
# never cached — because GameState.players can populate in any order relative
# to tiles (same Decision 061/062 hazard as board_rows).
...
func _on_tile_changed(coord_key: String) -> void:
    if _board_rows < 7:
        ...
        queue_redraw()
        return
    # Local player's baseY can arrive AFTER the first full repaint above (same
    # collection-ordering hazard as board_rows) — if the flip status just
    # became known/changed, every already-painted tile is stuck in the wrong
    # orientation, so force one more full repaint rather than just this tile.

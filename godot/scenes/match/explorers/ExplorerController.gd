...
## First spawn: set label/badge, snap position (no tween).
func setup(id: String, explorer_data: Dictionary, board_rows: int) -> void:
    explorer_id = id
    ...
    position = BoardCoord.to_world_position(target_coord, _board_rows)

## Subsequent updates: update badge, tween to the new position.
func update_from_state(explorer_data: Dictionary, board_rows: int) -> void:
    _board_rows = board_rows
    bot_badge.visible = explorer_data.get("isBot", false)
    _set_target(explorer_data)
    _tween_to_target()
...
func _tween_to_target() -> void:
    if _tween:
        _tween.kill()
    _tween = create_tween()
    _tween.tween_property(self, "position", BoardCoord.to_world_position(target_coord, _board_rows), TWEEN_DURATION)
